...
npm install
npm run compile      # 编译
npm test             # 编译 + 运行全部测试（11 套件 / 509 项断言，无需 VS Code 运行时）
...
题面里的图只看路径（不内联图片数据），需要时直接读 `local.assets` 里的文件即可。
...
│   ├── test/                 # 本地测试引擎（S6）：不依赖 VS Code，可脱离宿主单测
│   │   ├── toolchain.ts      # 工具链模型（屏蔽编译/解释差异）+ 内置三套
│   │   ├── compare.ts        # 严格逐字节比较 + 首个差异定位
│   │   ├── runner.ts         # prepare / run / compare 三步 + 结果与报告
│   │   ├── watchdog.ts       # 三闸看门狗（时间 / 输出体积 / 内存）
│   │   ├── terminal.ts       # 伪终端桥（CRLF 输出）
│   │   ├── tasks.ts          # 自定义 `oj` 任务
│   │   ├── wiring.ts         # 装配：选工具链、发现样例、拼路径
│   │   └── tools.ts          # MCP 三工具的服务层（零 vscode 依赖）
│   ├── config/               # 配置说明书与 AI 初始化（S6.5）
│   │   ├── manual.ts         # 单一真相源：结构来自 package.json，语义在此
│   │   ├── writer.ts         # 配置写入计划（纯函数）
│   │   └── tools.ts          # MCP get_config_manual / init_config
│   ├── workspace/            # 工作区侧装配
│   │   ├── initializer.ts    # 比赛项目初始化策略
│   │   ├── wiring.ts         # 初始化依赖接线
│   │   └── resources.ts      # 题目本地路径清单（S6.7，供 MCP 返回）
│   ├── mcp/                  # 对外层：MCP 服务器与工具
│   ├── media/                # 题面图片本地化（data URI 内联）
│   ├── utils/                # 基础层
│   │   ├── parser.ts         # 唯一 HTML 解析出口
│   │   ├── format.ts         # 唯一的 escapeHtml 实现 + 字节数格式化
│   │   ├── crypto.ts         # MD5 加密
│   │   ├── state.ts          # 唯一持久化出口
│   │   ├── config.ts
│   │   └── debug.ts
...
**`test/` 下的引擎不 import VS Code**（所以真编译真运行也能脱离宿主单测），
`buildReport()` 是「一次测试长什么样」的唯一说法（报告、结果页、MCP 都取它）。
...
| S6 | 🔶 | 本地测试引擎（S6.0–S6.7 已闭环：工具链 / 比较器 / 引擎 / 任务 / 本地测试结果页 / 提交结果页 / MCP 三工具）；剩工具链编辑页 |
...
S6 已闭环的能力（本地测试引擎）：**工具链抽象**（内置 C / C++ / Java / Python，可用
`.vsoj/toolchains.json` 覆盖或新增，引擎里没有任何语言特判）、**严格逐字节判定**
（先做换行归一化，等价站点 Linux 判题环境）、**三闸看门狗**（超时 / 输出体积 / 驻留内存，
kill 整棵进程树）、**编译器产物复用**（按「源文件 + 工具链 + 命令」的哈希）、
**两条平台无关的运行通道**（`oj` 类型自定义任务在终端里跑 / 命令与右键菜单）、

# OJ Platform VS Code Extension

OJ 在线判题平台 VS Code 插件，让你在 VS Code 内完成全部 OJ 操作：登录、浏览比赛、查看题目、编写与提交代码、查询判题状态。
...
│   ├── extension.ts          # 插件入口
│   ├── api/                  # API 层
│   │   ├── client.ts         # HTTP 客户端 + CookieJar
│   │   ├── auth.ts           # 登录/登出/Session
│   │   ├── contest.ts        # 比赛/题目列表
│   │   ├── problem.ts        # 题目详情
│   │   └── submit.ts         # 代码提交/状态查询
│   ├── views/                # 视图层

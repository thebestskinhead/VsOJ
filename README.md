...
未登录时插件只显示登录提示，不会拉取任何比赛、题目或判题数据。若想在离线状态下浏览已缓存的题目内容，可在设置里打开「强制离线」（`oj.cache.offline`）开关，它不需要登录。
...
**比赛目录建出来之前不写任何缓存**：只有你在确认框里点过「初始化并打开」、这个比赛的目录
建好之后，题面 / 样例 / 图片 / 列表缓存才会落盘。选「只看题面」不写任何文件 ——
相应的，那场比赛没有本地缓存（离线看不了，每次都要联网）。
...
└── <cid>-<比赛标题>/                            比赛项目文件夹（可见）
    ...
    └── problems/<目录名>/                      每道题一个目录（新题目为 <全局题号>-<标题>）
...
- **目录名一旦确定不再变化**：比赛目录为 `<cid>-<标题>`，题目目录为 `<全局题号>-<标题>`。
  站点后来改了标题、或在比赛里增删题目，都**不会**重命名已有目录，映射关系以
  `meta.json.problems` 为准
- **增删题目不会让你的代码错位**：题目的位置变了（题号整体后移 / 前移）时，本地目录与
  源码跟着题目本身走 —— 打开哪道题就编辑哪道题的目录，不会串到别的题上
...
4. **查看题目**：进入比赛后，"题目列表"显示所有题目，点击题目打开详情。打开一道还没保存到本地的题目时，会弹一个**居中的确认框**问**要不要把它保存到当前文件夹** —— 选「初始化并打开」才写盘；只要这个比赛的目录还没建好，下次打开题目（这道或别的题）还会再问一次（选「只看题面」就只读题面、不写任何文件）。目录一旦建好，这场比赛里点任何题都不再问
5. **编写代码**：在 VS Code 中正常编辑代码文件
6. **快捷操作**：打开题目后，侧边栏「题目列表」标题栏上的下拉里放着**提交代码**、**本地测试**、
   **编译当前题目**、**强制重新编译** —— 都是对当前题目的操作，没打开题目时会提示先打开一道题。
   编译或测试没能跑起来时（编译报错、找不到编译器）会自动打开结果页，把编译器原文与缺的命令列出来
7. **提交代码**：选中题目后，在编辑器中按 `Ctrl+Shift+S`，输入验证码提交
8. **查看结果**：提交后会自动打开「提交结果」页（把 `oj.statusViewMode` 设为 `webview`）——
...
参数（可选）：`cid` / `pid`、`source` - 源文件名（默认 `main.cpp`；只能是题目目录内的文件名，不能带路径）、
`rebuild` - 强制重新编译。
...
参数（可选）：`cid` / `pid`、`source` - 源文件名（默认 `main.cpp`）、`rebuild` - 强制重新编译。
...
**9. add_test_case** ⭐ 样例不够时自己补

给题目加一组测试用例（标准输入 + 期望输出），写入 `samples/<序号>.in` 与 `.out`；
之后 `run_local_test` 会把所有成对样例一起跑。站点样例只有一组、或想补边界数据时用它。

不传 `index` 会自动追加到最后一组之后；传 `index` 则覆盖 / 新建该序号。
注意 `samples/` 属于缓存目录：执行「清理缓存」会连同它一起删掉（源码与 `test/` 结果会保留）。

参数（可选）：`cid` / `pid`、`index` - 用例序号（从 1 开始，不传则追加）；
`input` - 标准输入全文、`output` - 期望输出全文（**这两个必填**，没有输入时传空字符串 `""`）。

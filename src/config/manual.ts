...
export const CONFIG_SEMANTICS: Record<string, ConfigSemantic> = {
  ...
  statusViewMode: {
    ...
    detail: '`browser` 打开站点原页面最完整，但会跳出编辑器；`webview` 是插件自绘的结果页，'
      ...
      + '`output` 是 Output 面板里的文本表格，同样会轮询待判定的提交，只是整屏重绘。',
    pitfall: '填了三个之外的值不会报错，会静默按 `browser` 走。',
    getter: 'getStatusViewMode',
  },
  statusPollInterval: {
    group: '平台接入',
    summary: '待判定提交的轮询起始间隔（毫秒）',
    detail: '`oj.statusViewMode` 为 `webview` 或 `output` 时都生效：'
      + '「等待 / 编译中 / 运行并评判」那几条会按这个间隔去查 `status-ajax.php`，'
      + '**每次翻倍、封顶 8 秒**。站点自己的状态页用 80ms 起步，那是页面直连同机 OJ 的量级；'
...
function renderPitfalls(entries: ConfigEntry[]): string {
  ...
  const unusedSection = unused.length
    ? [
      '### 声明了但当前版本没生效',
      '',
      ...unused.map((e) => `- \`${e.key}\`：${e.semantic!.summary}`),
      '',

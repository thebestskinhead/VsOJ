...
- **取数一律过闸门**：每个取数入口（`api/contest.ts`、`api/problem.ts`、`api/submit.ts`）
  ...
  **登录态或离线开关一变，屏幕上已渲染的内容立刻按现态重判或收回**（两个列表刷新、题面整页重走一次闸门、状态整页换成登录提示），不留到用户下次点击才同步——列表可以只留一行占位，题面与状态整页整屏都是内容，留着就等于「未登录也能看」。
...
`npm test` 一次性跑完全部套件（**32 套件 / 2456 项断言**），全部脱离 VS Code 运行时
...
| `access` | 151 | 访问闸门：登录态与离线的状态转换；未登录一律拒绝且零请求；离线仅强制开关授权读缓存；会话过期降级并广播；状态一变已渲染内容即收口 |

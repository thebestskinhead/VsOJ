...
- **取数一律过闸门**：每个取数入口（`api/contest.ts`、`api/problem.ts`、`api/submit.ts`）
  自己判「缓存能不能用」，调用方（`views/*`、`mcp/*`、初始化器）不必也不该自己记着先问缓存。
...
- **写操作不吃缓存**：提交 / 登录 / 取 CSRF 直连站点，且**提交成功即作废状态缓存** ——
  否则几秒后自动打开的状态页会命中 TTL 内的旧快照，看起来像「提交没生效」。
- **离线开关**：`oj.cache.offline` = 强制只读缓存；`oj.cache.enabled` = 是否写缓存。
  离线且本地连缓存都没有时**抛 `OfflineNoCacheError`（`code: OFFLINE_NO_CACHE`），
...
`npm test` 一次性跑完全部套件（**31 套件 / 2290 项断言**），全部脱离 VS Code 运行时

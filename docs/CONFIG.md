...
- **类型**：`number`｜**默认值**：`180`
- **取值**：整数；负数表示永不过期
- **说明**：管的是「命中且新鲜就直接用，**不发任何请求**」这条快路径，用于比赛列表 / 题目列表这类轻量数据。题目详情**不走**它（走 `staleSeconds`）。
- **示例**：`180`
...
- **类型**：`number`｜**默认值**：`900`
- **取值**：整数；负数表示永不过期
- **说明**：与 `ttlSeconds` 分工不同、**不冲突**：这里管的是「先渲染缓存，超过这个年龄才在后台重新拉」。题目详情页用的就是它。
- **示例**：`900`
...
- **说明**：适合断网/机房无网时翻已缓存过的题目。缓存里没有的内容会直接失败，不会偷偷联网。

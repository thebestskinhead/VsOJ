...
| S2 | 2026-09-13 | 会话保活 + 登录失效自愈 | `npm run test:session` 84 项断言 |
...
| S6.9 | 2026-09-14 | **题目快捷操作下拉**：侧边栏「题目列表」标题栏挂一个下拉，放提交代码 / 本地测试 / 编译当前题目 / 强制重新编译；未打开题目时三处提示统一为「请先打开一道题」 | `npm test` 28 套件 / 1435 项断言；`test:menu-contrib` 20 项 |
| S6.10 | 2026-09-14 | **写盘前确认 + 装配类失败一律打开结果页**：打开还没落盘的题时先问一次（初始化并打开 / 只看题面 / 本场不再问），回答只活在一次查看会话里；编译没过、工具链命令找不到时也打开结果页把原文与缺的命令摆出来（**S7.3 起**：判据改为「比赛目录是否存在」，弹窗改居中模态，去掉「本次不再问」） | `npm test` 全绿 |
| S6.11 | 2026-09-15 | **布局 v3 与题目索引按身份对齐**：题目目录由位置字母改为 `<全局题号>-<标题>`；题集插题 / 删题 / 重排时源码、样例、图片、测试历史不再整体错位一格 | `npm test` 全绿；新增 `test:sync` 30 项 |
...
**决策：会话级判定随登录作废**
保活模块里的探测结论（含验证时点与最近错误）、心跳失败计数、以及「已失效只上报一次」的去重标记，
都属于**上一个会话**的状态；重新登录换的是新会话，这些必须一并作废 —— 否则状态栏会一直挂着上一次探测的「登录已过期」，
直到下一次定时探测（最长一个探测间隔）才纠正；新会话再次失效时也会因为去重标记未复位而不再提示重新登录。
登录成功即视为一次有效验证（登录前 `AuthService.login()` 已用与探测相同的口径确认过），
因此不再多发一次请求，只记下验证时点并顺手接上心跳。
...
1. **缓存只存原始信息**（用户确认）。派生数据是「解析结果」，随时可从原始 HTML 重算，
   ...
   走 parser/api 按需产出。
2. **一律缓存优先，按数据性质分档**。列表与提交状态走同步 TTL（命中且新鲜零请求，
   过期才联网并落盘），题面走异步刷新（先渲染缓存、过期再后台补拉）；
   用户显式刷新与自己刚提交完的状态查询绕过 TTL。离线且无缓存时报明确错误
   （`OfflineNoCacheError`），不返回空结果冒充「站点上没有」。
...
## S6.11 — 布局 v3：题目索引按身份对齐（2026-09-15）

**要解决的问题**：题目目录名原先带**位置字母**（v2），而位置是会变的 —— 往题集中间插一道题，
后面所有题的位置整体后移一格，目录名里的字母跟着失效。若不处理，一道题的源码 / 样例 / 图片 /
测试历史会**整体串到隔壁题**。

**做了什么**
- 题目目录名改为 `<全局题号>-<标题>`（`LAYOUT_VERSION = 3`）。**全局题号与位置无关、逐题唯一**，
  插题 / 删题不会让它变化。
- 题目索引 `meta.json.problems` 改为按**身份**认领目录，身份优先级：
  `g:<全局题号>` → `t:<目录名里的题名比较键>` → `t:<标题比较键>`；目录名的题号前缀在比对题名前一律剥掉。
- 每轮由列表驱动的同步（`syncProblemIndex`）做三件事：按身份认领本地目录（**认领到就永远沿用该目录名**）、
  给新题开新目录、把站点上消失的题登记为孤儿（目录连同用户产物留着）。
  孤儿同样参与认领 —— 一道题消失又出现时会拿回原目录，而不是另起一个空目录。
- 落盘前只更新索引，**从不重命名、移动或删除已有目录**。

**验证**：新增 `test/sync.test.js`（30 项：插题后移 / 删题留孤儿 / `title` 被覆盖时按目录名认亲 /
同名多目录认产物多者）；后续又补 `test/sync-stress.test.js`（717 项，固定种子随机，
覆盖 8 种变更 × 60+ 轮）钉住不变量：**目录不乱认、用户产物零改动**。

---

## S7.1 — 取数统一过访问闸门（2026-09-16）

**要解决的问题**：站点自身不设防（公开比赛的 `problem.php` 匿名即渲染完整题面），
所以「未登录不给看」只能由本机把关；而此前这道把关散落在各个调用点，**事后看响应体已经晚了**。
离线判断也各写各的，离线且本地无缓存时会返回空结果，**把「离线拿不到」谎报成「站点上没有」**。

**做了什么**
- 新增 `src/session/access.ts`（**访问闸门**）：
  - 纯函数 `accessMode({ loggedIn, forcedOffline, reachable })` → `site` / `offline` / `login-required`。
    口径：**强制离线优先**；已登录恒 `site`（不看可达性）；未登录且明确不可达才是 `offline`，
    **可达性未知按可达处理**（放宽免登录缓存必须由「明确不可达」支撑，探测失败也算可达）。
  - `decideAccess(target, mode, hasCache)` → `site` / `cache` / `deny(LOGIN_REQUIRED | OFFLINE_NO_CACHE | OFFLINE_WRITE)`；
    写操作（`submit`）离线**恒拒**。
  - `AccessGate` 承载带状态的部分：只在离线态才探缓存；会话失效广播去重（`sessionRenewed()` 复位）。
- 三个取数入口（`api/{contest,problem,submit}.ts`）**第一步就过闸门**，取回正文后再 `noteResponse()`
  复核是否为登录页 —— 未登录时**一个请求都不发**。
- **未登录一律只走登录提示**：比赛列表、题目列表、题面、判题状态、提交都不例外。
- **免登录读缓存的许可只来自用户主动打开的强制离线开关**（`oj.cache.offline`），
  可达性探测结论与传输失败**都不能**作为授权来源。
- **离线且无缓存抛 `OfflineNoCacheError`**（`code: OFFLINE_NO_CACHE`），而不是返回空结果。
- **状态一变即收口**：登录态、离线开关、工作区一变，屏幕上已渲染的内容立刻按现态重判或收回
  （两个列表刷新、题面整页重走闸门、状态整页换成登录提示），不留到下次点击。

**验证**：新增 `test/access.test.js`（166 项：真值表穷举、请求记账钉死「未登录零请求」、
状态转换端到端、离线特例）；扩写 `test/cache-gate.test.js`（80 项）。
`npm test` → **32 套件 / 2481 项断言**全绿。

---

## S7.2 — MCP 补测试用例工具 + 指定编译文件（2026-09-16）

**要解决的问题**：AI 侧「改代码 → 本地验证 → 再改」闭环已经能用，但**测试数据是死的** ——
样例只有站点给的那一组，想验边界（空输入、最大规模、多行输出）只能让用户手工往
`samples/` 里塞文件。另外 `compile_problem` / `run_local_test` 只会编译
`oj.project.sourceFileName`（默认 `main.cpp`），一份题解想拆成多个源文件
（或同时留个暴力 / 对拍程序）就没法跑。

**做了什么**

- **新增 MCP 工具 `add_test_case`**（`src/test/tools.ts` + `src/mcp/tools.ts`）：
  - 参数 `input` / `output`（**必填**，各自可为空字符串）+ 可选 `index`；
  - **不传 `index` 时追加到最后一组之后**；传了就写入该序号（覆盖并如实回执「覆盖原有用例」）；
  - 回执给出两条落盘路径、当前**成对可跑**的序号清单、以及半对（缺 `.out`）的明细 ——
    AI 需要知道下一组该用几号；
  - 写盘走 `CacheStore.writeUserFile` 而不是 `writeSamples`：手写的用例是**用户资产**，
    不该被「`oj.cache.enabled=false`（不写站点数据）」连坐挡掉。
- **`compile_problem` / `run_local_test` 支持 `source`**（`src/test/wiring.ts`）：
  默认仍是 `oj.project.sourceFileName`（通常 `main.cpp`）；指定后编译 / 运行的就是它。
  该参数**只接受题目目录内的文件名** —— 带路径分隔符、`..`、绝对路径（含 Windows 盘符）
  一律拒绝并说明理由：它是便利参数，不该变成「让编译器去读题目录之外文件」的通道。
- 分层不变：路径拼装仍在 `cache/paths`（`add_test_case` 只认序号与内容，
  路径与写盘由扩展侧注入），引擎与工具链层零改动。

**验证**：`test/mcp-test-tools.test.js` 91 → **113 项**（新增：自动追加序号、显式覆盖、
非法序号 / 缺参数拒绝、目录未建立的可操作文案、补完用例后 `run_local_test` 真的跑它、
`source` 生效且不退回 `main.cpp`、带路径 / 绝对路径的来源被拒）；
`test/config-tools.test.js` 的工具总数断言 8 → 9。
`npm test` → **32 套件 / 2503 项断言**全绿（真实 `McpToolHandler` + 真 g++）。

---

## S7.3 — 写盘确认改判据 + 确认框居中（2026-09-16）

**要解决的问题**：S6.10 的写盘确认有两个别扭处。

1. **「已同意」是内存里的第二份真相**：`InitConfirmations` 记「这次会话里点过头」，
   而「这个比赛到底写没写过盘」磁盘上其实已经有答案（目录在不在）。
   两份状态一旦不一致（目录被手动删掉、会话中途重载窗口等），就会出现
   「目录没了却不再问」或「目录还在却反复问」。
2. **确认框落在右下角**（`modal: false`）：它混在通知流里，容易被随后进来的
   构建 / 语言服务提示挤掉 —— 而这是一次**写盘授权**，不该这样被稀释。

**做了什么**

- **判据唯一化**：`decideOpenProblem` 需要征求同意的条件由 `!f.initConfirmed`
  改为 `!f.contestInitialized`，也就是「要新建比赛根目录了」：
  - 目录不存在 → **每次打开题目都重新问一次**；
  - 目录已存在 → 以前同意过并写盘成功，视为已同意，永不再问。
- **删除 `InitConfirmations` 类与 `WorkspaceFacts.initConfirmed` 字段**；
  `beginContestSession` 不再重置「已同意」（它只保留 D19 的「暂不」重置）。
- **确认框改居中模态**：`askInitConfirm` 用 `{ modal: true }`，按钮简化为**两个**
  （初始化并打开 / 只看题面）——「本次不再问」在新模型下名不副实（下次照样会问），
  遂移除；关闭弹窗仍等于「只看题面」（最保守的默认）。
- 侧边栏的「暂不」（`InitEntryDismissals`，D19）**保持不变** —— 它管条目可见性，
  与写盘许可不是一回事。
...
优先级与依赖见 `docs/ARCHITECTURE.md` §4。**S0–S7（含 S6.10、S6.11、S7.1）已全部闭环**：
...
- ✅ **S5 比赛项目初始化** —— 已完成并提交（`302c29f` → `8befccd` → `97cf269` →
  ...
  比赛目录建在 **workspace 可见根**（`<cid>-<标题>/`），题目目录用 `<全局题号>-<标题>/`。
- ✅ **S6 本地测试引擎 + MCP 扩展** —— 已完成 S6.0–S6.10：
  ...
  - ✅ **题目快捷操作下拉**（S6.9）：侧边栏「题目列表」标题栏挂下拉，放提交代码 / 本地测试 /
    编译当前题目 / 强制重新编译；未打开题目时统一提示「请先打开一道题」
  - ✅ **写盘前确认 + 装配类失败开页**（S6.10；**S7.3 起**判据改为「比赛目录是否存在」、
    弹窗居中、去掉「本次不再问」）：初始化前先征求同意；编译没过、工具链命令找不到时也打开结果页
  - MCP 工具现状：共 **9 个**（5 个只读 / 配置类 + 4 个测试工具：`compile_problem` / `run_local_test` / `add_test_case` / `get_last_test_result`）。

...
- 新增 `src/workspace/initializer.ts`：进入比赛时建目录、写题目列表、逐题拉原始题面、
  **把 `sampleInput/sampleOutput` 落成 `samples/1.in` / `1.out`**（接上 `writeSamples`）、
  拉取题面图片到 `assets/`、回填 `meta.json`。幂等 / 增量。
- 在 `oj.enterContest` 挂触发点（S4 刻意未做，保持语义解耦）。
- 触发时机、派生文件边界、预取深度、重复进入行为 **待用户确认**（见提问）。
...
优先级与依赖见 `docs/ARCHITECTURE.md` §4。**S4 已闭环**，当前推进 S5：

- ✅ **S4 运行期缓存刷新与离线预览** —— 已完成并提交（`ffcf7f9` → `8f822e7` →
  `1e533e1` → `8a52ac0` → `49e9cf8`），计划见 `docs/PLAN_S4.md`。
- **S5 比赛目录初始化** —— 已细化为 `docs/PLAN_S5.md`（含 12 条已确认决策、5 项关键设计、
  S5.0–S5.6 阶段切分、11 条行为契约、测试策略，以及 4 项待拍板事项）。
  **与 S4 在语义上解耦**：初始化是阶段性的"建立工作区"动作，运行期刷新是"保持数据新鲜"，
  两者不共用触发路径，S4 未引入任何"进入比赛即触发"的代码。
  动机：用户要的是 **LeetCode 式流程**（点题 → 自动切到源文件、左代码右题目），
  以及一键本地测试，二者都**依赖磁盘上有文件**，所以初始化是它们的地基而非可选优化。
- **S6 本地测试引擎 + MCP 扩展** —— 题目图片返回、样例识别、以 exe 为输入的一键本地测试，
  并支持通过 `.vscode/tasks.json` 接入编辑器流水线。MCP 现有 3 个工具
  （`get_contest_problems` / `get_current_problem` / `get_contest_list`）**尚未扩展**。

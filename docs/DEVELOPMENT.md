...
npm install
npm run compile      # 编译
npm test             # 编译 + 运行全部测试（32 套件 / 2509 项断言，无需 VS Code 运行时）
...
│   │   ├── freshness.ts      # 纯函数：缓存多旧 / 算不算过期 / 人类可读时长
│   │   ├── revalidate.ts     # 重访决策（offline / no-cache / fresh / stale / unreachable）
│   │   └── refresher.ts      # 单题 / 全量强制刷新执行器（只执行、不管 UI）
│   ├── session/              # 会话层：保活、失效自愈与访问闸门
│   │   ├── guard.ts          # 失效判定（唯一判定点）+ 意图重放
│   │   ├── keeper.ts         # 心跳保活与登录态探测
│   │   ├── connectivity.ts   # 网络可达性探测（带结果缓存，只回答能不能上网）
│   │   └── access.ts         # 访问闸门：登录态/离线裁决，取数前的本机把关（未登录一律走登录提示）
...
取数统一过缓存闸门（见 `ARCHITECTURE.md` §3.2）：未登录一律只进登录提示、不访问任何内容；
免登录读本地缓存仅当用户主动打开强制离线开关，可达性探测与传输失败都**不**构成授权。
列表 / 状态走同步 TTL，题面走异步刷新，离线且无缓存时报明确错误而不是给空结果。
...
S6.10 已闭环的能力：打开一道还没落盘的题时先征求同意再写盘；编译没过、
工具链命令找不到这类「装配类失败」也一律打开结果页，把编译器原文与缺的命令摆出来。

S7.3 已闭环的能力：写盘许可**不再单独存状态** —— 以「比赛项目目录是否存在」为准。
需要新建根目录时每次都弹**居中的模态确认框**（`modal: true`，避免混在右下角通知流里被挤掉），
目录一旦建好就视为已同意、不再问；选「只看题面」不写盘，下次打开还会再问。
`InitConfirmations` 类与「本次不再问」按钮随之移除。

S7.4 已闭环的能力：**比赛根目录只由「同意初始化」创建** —— 缓存写穿
（`write*Html` / `writeSamples` / `writeProblemAsset`）与 `syncProblemIndex` 都先
`resolveContestDir`，目录不存在就跳过（不再顺手把目录建出来）。否则删掉目录后
任何一次列表刷新都会让它「复活」，S7.3 的判据就永远等不到那次确认；
「只看题面不写任何文件」也才真的成立。代价是未初始化的比赛没有本地缓存。

S6.11（布局 v3）与 S7.1（访问闸门）已闭环的能力：题目目录改用 `<全局题号>-<标题>`，
索引按**身份**认领目录，题集插题 / 删题 / 重排不再让源码与样例错位（`test/sync.test.js`）；
取数**一律过闸门** —— 每个取数入口先问 `session/access.ts`「此刻允不允许取数、缓存能不能用」，
未登录一律只进登录提示且零请求，免登录读缓存只由用户主动打开的强制离线开关授权，
离线且无缓存时抛 `OfflineNoCacheError` 而不是给空结果；登录态 / 离线开关一变，
屏幕上已渲染的内容立刻按现态重判或收回（`test/access.test.js`、`test/cache-gate.test.js`）。

S7.2 已闭环的能力：MCP 新增 `add_test_case` —— AI 可以自己往 `samples/` 补测试用例
（不传序号则追加到末尾之后，传了则覆盖该序号），补完 `run_local_test` 会一起跑；
`compile_problem` / `run_local_test` 支持 `source` 参数指定编译文件（默认 `main.cpp`，
只接受题目目录内的文件名，带路径或 `..` 一律拒绝）。
